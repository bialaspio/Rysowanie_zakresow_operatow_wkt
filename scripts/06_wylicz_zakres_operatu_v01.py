...
def add_tr_coment(operat_baza):
	"""+'"'+operat_baza+'"'+"""
	craete_f_query = """CREATE OR REPLACE FUNCTION partia_05.wylicz_wkt_operat_"""+operat_baza+"""() RETURNS TRIGGER AS $wylicz_wkt_operat$
					BEGIN
						IF EXISTS(select 1 from ( 
												select count (status) as licz from( 
													select status from partia_05."""+'"'+operat_baza+'"'+""" where status = '06_Przelicz_zakres'
												) as foo where status = '06_Przelicz_zakres'
								) as foo2 where licz >0
						) 
						THEN
							-- usuniecie blednych typow gemetrii
							...
							-- przypisanie statusu 04 tam gdzie jest uzytkownik i jest zmionana kolumna opr_zakres
							UPDATE partia_05."""+'"'+operat_baza+'"'+""" SET status='04_Zakres_operatu' where uzytkownik is not null and opr_zakres is not null;
							-- uzupelnienie tam gdzie mozna nic nie wypelanaic
							update partia_05."""+'"'+operat_baza+'"'+""" SET 
							status='04_Zakres_operatu',
							uzytkownik = 'Admin',
							opr_zakres = 'NIE' 
							where 
							opr_zakres is null 
							and
							(ST_Equals (wkt_geometry, 'POLYGON ((6581730.228609123 5551850.15233694, 6581734.392691579 5551850.045565596, 6581734.392691579 5551850.045565596, 6581734.499462924 5551845.988254485, 6581730.228609123 5551845.988254485, 6581730.228609123 5551850.15233694))'::geometry)
							or 
							ST_IsEmpty(wkt_geometry));
							--  ustawienie danych dla rekordu operatu 
							UPDATE partia_05."""+'"'+operat_baza+'"'+""" SET status='04_Zakres_operatu',uzytkownik='Admin' where plik like 'operat';
							-- usuniecie z geometrii danych ioniocajclizacyjnych  
						...
						END IF;
					END;
				$wylicz_wkt_operat$ LANGUAGE 'plpgsql';"""
				
	exec_query_commit_PG(craete_f_query)

	tr_query ="""DROP TRIGGER IF EXISTS wylicz_wkt_"""+operat_baza+""" ON partia_05."""+'"'+operat_baza+'"'+""";"""
	exec_query_commit_PG(tr_query)
	
	crate_t_query ="""CREATE TRIGGER wylicz_wkt_""" +operat_baza+"""
					AFTER UPDATE 
						ON partia_05.""" +'"'+operat_baza+'"'+"""
					   FOR EACH ROW
					EXECUTE PROCEDURE partia_05.wylicz_wkt_operat_"""+operat_baza+"""();"""
